...
export function TitleScreen() {
  ...
  // Pause the WebGL render loop when the title scrolls out of view (big perf win).
  useEffect(() => {
    const el = sectionRef.current;
    if (!el) return;
    const io = new IntersectionObserver(([e]) => setVisible(e.isIntersecting), { threshold: 0.04 });
    io.observe(el);
    return () => io.disconnect();
  }, []);

  return (
    <section ref={sectionRef} id="title" aria-label="Title screen" className="relative h-screen w-full overflow-hidden bg-[#0c0f16]">
      <div className="absolute inset-0">
        {show3d ? (
          <HeroBoundary fallback={<DragonAtmosphere />}>
            <DragonScene active={visible} />
          </HeroBoundary>
        ) : (
          <DragonAtmosphere />
        )}
