    ...
    <>
      <header className="fixed top-0 inset-x-0 z-[75] nb-border border-x-0 border-t-0 bg-crt/95 backdrop-blur">
        <div className="mx-auto max-w-7xl flex items-center gap-3 px-3 py-2 font-pixel text-[10px] text-paper">
          ...
          <button type="button" onClick={() => window.dispatchEvent(new Event("open-cmdk"))}
            aria-label="Open command palette" title="Quick jump (⌘K)"
            className="hidden sm:inline-flex nb-border px-2 py-1 bg-pop-magenta text-ink items-center gap-1">⌘K</button>
